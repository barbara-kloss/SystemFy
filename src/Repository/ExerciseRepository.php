...
class ExerciseRepository
{
    ...
    {
        $sql = "INSERT INTO exercise(id_user, 
         peso, repeticao, 
         tipo_exercicio, objetivo, dia, 
         observacao, categoria, id_personal, video) values(?,?,?,?,?,?,?,?,?,?);";
        $stmt = $this->pdo->prepare($sql);
        $stmt->bindValue(1, $exercise->id_user);
        $stmt->bindValue(2, $exercise->peso);
        $stmt->bindValue(3, $exercise->repeticao);
        $stmt->bindValue(4, $exercise->tipo_exercicio);
        $stmt->bindValue(5, $exercise->objetivo);
        $stmt->bindValue(6, $exercise->dia);
        $stmt->bindValue(7, $exercise->observacao);
        $stmt->bindValue(8, $exercise->categoria);
        $stmt->bindValue(9, $exercise->id_personal);
        $stmt->bindValue(10, $exercise->video);
