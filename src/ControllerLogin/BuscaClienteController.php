...
class UserRepository
{
    ...
    {
        // Certifique-se de que a conexão PDO está sendo obtida corretamente
    ...
    public function add(User $user) : bool
    {
        ...
        // ... (binding de valores omitido por brevidade)
    ...
    // =========================================================
    // NOVO MÉTODO: BUSCAR CLIENTES POR NOME OU EMAIL
    // =========================================================
    public function buscarPorTermo(string $termo): array
    {
        // Seleciona as colunas necessárias para carregar no formulário
        $sql = "SELECT id, nome_completo, data_nascimento, genero, telefone, email, 
                       altura, peso, objetivos, observacao, massa, gordura, plano_id, peso_meta
                FROM user
                WHERE (nome_completo LIKE :termoNome OR email LIKE :termoEmail) 
                AND permissao = 'cliente' 
                LIMIT 10"; // Limitar a 10 resultados é bom para a performance

        $stmt = $this->pdo->prepare($sql);
        $likeTerm = '%' . $termo . '%';
        
        // Define os valores para o LIKE
        $stmt->bindValue(':termoNome', $likeTerm, PDO::PARAM_STR);
        $stmt->bindValue(':termoEmail', $likeTerm, PDO::PARAM_STR);
        $stmt->execute();
        
        // Retorna um array de dados associativos, pronto para ser transformado em JSON
        return $stmt->fetchAll(PDO::FETCH_ASSOC); 
    }
    
    // ... (Métodos update, all, find, hydrateUser omitidos por brevidade)
